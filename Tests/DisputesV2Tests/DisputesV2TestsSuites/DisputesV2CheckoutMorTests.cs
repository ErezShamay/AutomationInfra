...
[TestFixture]
[AllureNUnit]
[AllureSuite("CheckoutMorDisputesV2Tests")]
[AllureDisplayIgnored]
[Parallelizable(ParallelScope.All)]
public class DisputesV2CheckoutMorTests
{
    ...
    public DisputesV2CheckoutMorTests()
    {
    ...
    [Category("CheckoutMorDisputesV2Tests")]
    [Test(Description = "TestValidateCheckoutMorDisputesV2Won with generated merchant"), CancelAfter(720 * 1000)]
    public async Task TestValidateCheckoutMorDisputesV2Won()
    {
        try
        {
            Console.WriteLine("\nStarting TestValidateCheckoutMorDisputesV2Won");
            var (installmentPlan, _) = await InitPlan(41.80);
            var fullPlanInfoAfter = await _fullPlanInfoIpn.SendGetFullPlanInfoIpnAsync(_requestHeader!,
                ...
                gatewayEventTypeList: new List<string> { "payment_captured", "dispute_received", "dispute_won" });
            ...
            Console.WriteLine("TestValidateCheckoutMorDisputesV2Won is Done\n");
        }
        catch (Exception exception)
        {
            Assert.Fail("Error in TestValidateCheckoutMorDisputesV2Won\n" + exception + "\n");
        }
    }

    [Category("CheckoutMorDisputesV2Tests")]
    [Test(Description = "TestValidateCheckoutMorDisputesV2Lost with generated merchant"), CancelAfter(720 * 1000)]
    public async Task TestValidateCheckoutMorDisputesV2Lost()
    {
        try
        {
            Console.WriteLine("\nStarting TestValidateCheckoutMorDisputesV2Lost");
            ...
            var jResponseDisputesResponse = await _postDisputesFunctionality.SendPostRequestPostDisputesAsync(
                _requestHeader!, installmentPlan.InstallmentPlanNumber,
                jResponseFullPlanInfo.InstallmentPlan.Merchant.Id, "Lost");
            ...
            Console.WriteLine("TestValidateCheckoutMorDisputesV2Lost is Done\n");
        }
        catch (Exception exception)
        {
            Assert.Fail("Error in TestValidateCheckoutMorDisputesV2Lost\n" + exception + "\n");
        }
    }
    ...
    private async Task<(ResponseV3.ResponseRoot installmentPlan, string? pgtlCaptureTransactionId)> InitPlan(double amount)
    {
        try
        {
            ...
            var installmentPlan = await _installmentPlans.CreatePlanAsync(Environment.GetEnvironmentVariable("ApiV3")!, _requestHeader!,
                "Active", 4, Environment.GetEnvironmentVariable("CheckoutMORTerminal")!,
