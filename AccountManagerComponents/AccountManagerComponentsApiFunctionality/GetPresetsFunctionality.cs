...
public class GetPresetsFunctionality
{
    private const string EndPoint = "/api/v1/Presets";
    private readonly HttpSender _httpSender = new();
    private readonly EnvConfig _envConfig = new();
    
    public async Task<GetPresetsResponse.Root> SendGetRequestGetPresets(
        RequestHeader requestHeader)
    {
        try
        {
            Console.WriteLine("\nStarting SendGetRequestGetPresets");
            var response = await _httpSender.SendGetHttpsRequestAsync(_envConfig.Ams + EndPoint, 
                requestHeader);
            var jResponse = JsonConvert.DeserializeObject<GetPresetsResponse.Root>(response);
            Console.WriteLine("Done with SendGetRequestGetPresets\n");
            return jResponse!;
        }
        catch (Exception exception)
        {
            Console.WriteLine("Error in SendGetRequestGetPresets \n" + exception + "\n");
            throw;
        }
    }
}